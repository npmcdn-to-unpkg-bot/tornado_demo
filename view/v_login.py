...
from config import ldap_config, private_key
...
class Login(Base):
    ...
    def post(self):
        ...
        password = base_misc.t_decrypt(unquote(password), private_key)

        if account and password:
            # 域验证
            msg = base_misc.check_login(ldap_config['ldap_host'], account, password)
            if msg == '3':
